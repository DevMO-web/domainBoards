...
function grabData(){
    ...
    .then((dataArray) => {
        if(display){
            ...
            .map(({productName, productPrice, id, mainImage}) =>  {
             return `
...
class Cart{
    ...
    transferCart(){
        if (this.cartItems.length === 0) {
            ...
            cartPointer.setAttribute('data-id', 'cartEMEMEMEMEMEMEME')
           
            
        } else {
            //redefine so that its no longer undefined
           
            let test = this.cartItems
            console.log(typeof(this.cartItems))
            const cartHTML = test.map(({ name, price, id, img, qty }) => {
                return `
                            
                        <li class="global-shopping-cart-li" data-id="${id}" data-description="removal">
                            <div class="global-shopping-cart-li-image-container">
                            ...
                            <div class="global-shopping-cart-li-content-container" data-info="products">
                                <h4 class="product-name" data-product-title="${name}">${name}</h4>
                                    ...
                                    <p class="product-qty" data-product-qty="${qty}">${qty}</p>
                                </div>
            ...
            console.log(test)
            const dataTable = test.map(({ name, price, id, img, qty }) => {
                return `
...
function CreateElement(item) {
    ...
    parentLi.append(thridDiv,firstDiv,secondDiv,);
    ul.append(parentLi)  
    }
